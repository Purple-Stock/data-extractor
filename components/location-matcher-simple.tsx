...
export function LocationMatcherSimple() {
  ...
  // Update preview content when matched data changes
  useEffect(() => {
    updatePreviewContent()
  }, [matchedData, activeTab])
  ...
  const updatePreviewContent = () => {
    ...
    const header = "CODIGO;DESCRIÇÃO"
    const rows = dataToPreview.map((row) => {
      return `${row["Cód. Auxiliar"] || ""};${row["Descrição_1"] || ""}`
    })
  ...
  const generateSimpleContent = (data: MatchedRow[]) => {
    let content = "CODIGO;DESCRIÇÃO\n"
    data.forEach((row) => {
      const line = `${row["Cód. Auxiliar"] || ""};${row["Descrição_1"] || ""}`
      content += line + "\n"
    })
    return content
  }
  ...
  const downloadAllLocationFiles = async () => {
    ...
    try {
      ...
      zip.file(`codigo_descricao_TODOS.txt`, generateSimpleContent(matchedData))
      ...
      locations.forEach((location) => {
        ...
        zip.file(`codigo_descricao_${safeLocationName}.txt`, generateSimpleContent(locationData))
      })
      ...
      const link = document.createElement("a")
      link.href = url
      link.download = `codigo_descricao_por_secao.zip`
      document.body.appendChild(link)
      link.click()
      document.body.removeChild(link)
      URL.revokeObjectURL(url)
  ...
  const downloadLocationFile = (location: string) => {
    ...
    const content = generateSimpleContent(locationData)
    downloadFile(content, `codigo_descricao_${safeLocationName}.txt`)
  }
    ...
    <div className="mx-auto max-w-6xl">
      ...
      <div className="text-center mb-8">
        ...
        <h1 className="text-4xl md:text-5xl font-bold bg-gradient-to-r from-slate-900 via-slate-800 to-slate-700 dark:from-slate-100 dark:via-slate-200 dark:to-slate-300 bg-clip-text text-transparent mb-4">
          INVENTÁRIO CONTAGEM SIMPLES
        </h1>
        <p className="text-lg text-muted-foreground max-w-2xl mx-auto">
          Faça upload dos arquivos Excel para comparar, combinar dados e gerar arquivos com apenas CÓDIGO e DESCRIÇÃO
        </p>
      </div>
        ...
        {matchedData.length > 0 && (
          <div className="mt-8">
            <div className="flex flex-col md:flex-row md:justify-between md:items-center gap-4 mb-4">
              <h3 className="text-lg font-medium">Resultados por Seção (CÓDIGO e DESCRIÇÃO)</h3>
